...
Builders submit Learnings containing operational knowledge derived from real-world task execution. Each submission is automatically quality-scored and screened for content and sensitivity, and is then either held in the Builder's review queue for the Builder's approval or published to the catalog on submission, depending on the screening result, the submission channel, and whether the Builder has activated standing publication consent under Section 5.9.3(g); a new account's first Learning is additionally held for operator review. Consumers search the catalog, discover relevant Learnings, and unlock full content by paying the current price, which is set initially by the Builder or calculated by Auxilo's pricing engine and then moved toward what the engine currently computes.
...
The Platform's software includes a chat history extraction pipeline (`POST /pipeline/upload`) designed to accept conversation text from Builders for automated extraction of Learnings. That pipeline exists in Auxilo's software but is disabled and not in operation; no conversation text can be submitted to Auxilo through it. If it is ever activated, these Terms and the Privacy Policy will be updated before use.

The representations and warranties in this Section 5.8 apply to conversation content uploaded through that pipeline at any time it is in operation, and apply to Learnings submitted through Autonomous Extraction as provided in Section 5.9.3(c). By uploading conversation content, you represent and warrant that:
...
While the pipeline is in operation, the Platform employs automated sensitivity scanning on all uploaded conversation content prior to processing. This scanning is a reasonable precaution and does not constitute a guarantee that all sensitive content will be detected. **The builder bears sole responsibility for ensuring uploaded content complies with these requirements, regardless of the outcome of any automated scan.** See Section 9.2 for the general sensitivity filter disclaimer.

### 5.9 Data Processing for Conversation Uploads

Sections 5.9.1 and 5.9.2 describe the data handling that applies to conversation content uploaded through the pipeline described in Section 5.8. Because that pipeline is disabled and not in operation, those two subsections are inoperative, and they apply only to conversation content uploaded while the pipeline was in operation. Section 5.9.3 (Autonomous Learning Extraction) and Section 5.9.4 (Retraction Right) are in operation and are not limited by this paragraph.
...
The Platform offers an autonomous extraction feature ("Autonomous Extraction") that allows Builders to enable continuous, hands-off generation of Learnings from their AI session transcripts. When enabled, the supported client integration processes each session transcript locally on the Builder's machine: it applies a client-side redaction pass, runs the extraction step through the Builder's own model client under the Builder's own agreement with their model provider, whether that model client is a supported coding-client integration or a directly-configured provider connection using a Builder-supplied API key (`auxilo provider set`), and submits only the resulting draft Learnings to Auxilo (`POST /learn`), where they are held in the Builder's own review queue until the Builder publishes them, except as provided in subsection (g). Session transcripts, raw or redacted, are not transmitted to Auxilo. An optional server-side extraction path exists in Auxilo's software but is disabled and not in operation; if it is ever activated, these Terms and the Privacy Policy will be updated before use. A current list of supported client integrations is maintained at https://auxilo.io/legal/supported-clients.
...
**(a) Seven-day retraction window.** A Builder may retract any Learning they published, however it was submitted, for a period of seven (7) days following the date the Learning was published. For a Learning that was held for review before publication, the publication date is the date it was approved, whether by the Builder from their review queue or by an Auxilo operator. For a Learning that was published when it was submitted — including a Learning published under standing publication consent (Section 5.9.3(g)) — the publication date is the date of submission. Retraction is effected by request through the Platform's catalog management interface or by email to hello@auxilo.io identifying the Learning. Upon a valid retraction request, Auxilo will remove the Learning from public discovery, search results, and the catalog API within a commercially reasonable time.
