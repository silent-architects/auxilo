...
/**
 * SepoliaE2E — one-shot live exercise of AuxiloSplitRouter on Base Sepolia.
 *
 * One funded key (env DEPLOYER_PK) plays deployer + settler + buyer-funder.
 * The script: deploys the router, funds a throwaway buyer with 1 USDC, has the
 * buyer sign a ReceiveWithAuthorization off-chain (vm.sign — no buyer ETH
 * needed, the buyer never sends a transaction), then settles via
 * settleAndSplitReceive and asserts the 70/30 split landed.
 *
 * Env:
...
contract SepoliaE2E is Script {
    ...
    function run() external {
        ...
        // 1. Deploy router: usdc = Base Sepolia USDC, settler = deployer.
        AuxiloSplitRouter router = new AuxiloSplitRouter(USDC, feeWallet, deployer);
        ...
        // 3. Buyer signs ReceiveWithAuthorization OFF-CHAIN over the DERIVED
        //    nonce (keccak256(abi.encode(contributor, 7000, salt))), using the
        //    real token's on-chain DOMAIN_SEPARATOR(). vm.sign is a cheatcode —
        //    no broadcast, no buyer gas.
    ...
    function _loadActors() internal {
        ...
        buyerPk = vm.envOr("BUYER_PK", uint256(keccak256(abi.encodePacked("auxilo-sepolia-e2e/buyer", deployer))));
        buyer = vm.addr(buyerPk);
        feeWallet = vm.envOr(
            "FEE_WALLET", vm.addr(uint256(keccak256(abi.encodePacked("auxilo-sepolia-e2e/feeWallet", deployer))))
        );
        contributor = vm.addr(uint256(keccak256(abi.encodePacked("auxilo-sepolia-e2e/contributor", deployer))));
        salt = vm.envOr(
            "E2E_SALT", keccak256(abi.encodePacked("auxilo-sepolia-e2e", deployer, block.timestamp, block.number))
        );
    ...
    function _preflight() internal view {
        require(
            ...
            "PRE-FLIGHT: deployer needs Base Sepolia ETH (>=0.00005; whole E2E is ~0.00001 at Base Sepolia's ~0.006 gwei). Fund via a Base Sepolia faucet."
        );
        require(
    ...
    function _assertAndReport(AuxiloSplitRouter router, uint256 contributorBefore, uint256 feeBefore) internal view {
        ...
        console2.log("");
        console2.log("Tx hashes: broadcast/SepoliaE2E.s.sol/84532/run-latest.json");
    }
}
