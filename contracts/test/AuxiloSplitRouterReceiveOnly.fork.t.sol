...
/**
 * Fork tests against REAL Base mainnet USDC (FiatTokenV2_2 behind a ZeppelinOS
 * proxy). Skips gracefully (vm.skip) when the "base" RPC endpoint is unreachable.
 *
 * The EIP-712 digest is built from the token's OWN DOMAIN_SEPARATOR() — read
 * directly rather than reconstructed.
 */
contract AuxiloSplitRouterReceiveOnlyForkTest is Test {
    ...
    function setUp() public {
        ...
        router = new AuxiloSplitRouterReceiveOnly(USDC, feeWallet, settler);
    ...
    /// tampered bps). The derived nonce changes, so FiatTokenV2_2's own signature
    /// verification must revert. Since this is the ONLY path, max settler-
    /// extractable is EXACTLY 0 for the whole contract.
    function test_Fork_SettlerCompromise_RevertsInRealUsdc() public {
        ...
        // A settler-named contributor (an arbitrary attacker address) → derived
        // nonce differs → real USDC rejects. Use a distinct honest-looking addr so
        // this is not caught by the B5 guard first (proving USDC's sig check).
        address attacker = makeAddr("fork-attacker");
        vm.prank(settler);
        vm.expectRevert(bytes("FiatTokenV2: invalid signature"));
        router.settleAndSplitReceive(buyer, value, 0, type(uint256).max, salt, sig, attacker, bps);
        ...
        assertEq(usdc.balanceOf(attacker), 0, "attacker got exactly 0");
    }
}
