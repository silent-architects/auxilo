...
function appendCleanLaneRow({ accountId, action, minAutoPublishQuality, tosVersionAtGrant,
  ipRedacted, userAgent, acceptPath, reason, stats } = {}) {
  ...
  try {
    ...
    appendAuditRow({
      ...
      source: {
        ...
        ...(action === 'grant' && { min_auto_publish_quality: row.min_auto_publish_quality, tos_version_at_grant: row.tos_version_at_grant, affirmed: true }),
