...
/**
 * EXTRACT-PER-CLIENT W1 PART C: the quality-score clamp above (AUD19-6 floor,
 * the per-grant min_auto_publish_quality) was calibrated against ONE model's
 * self-assessment behavior (claude-code). It cannot detect a different
 * provider/model inflating its own quality_self_assessment — so a submission
 * whose extraction_model.provider is not in this allowlist NEVER auto-
 * publishes through the clean lane, regardless of flag/consent/quality; it
 * always lands in the review lane instead. Extend this list only after a
 * calibration pass on the new provider's self-assessment behavior — not on
 * "it shipped a runModel module."
 */
const CLEAN_LANE_CALIBRATED_PROVIDERS = Object.freeze(['claude-code']);
const HOLD_UNCALIBRATED_PROVIDER = 'uncalibrated_extraction_provider';
/**
 * EXTRACTION-MODEL-PROVENANCE (PUNCH-LIST P1): distinct hold reason for a
 * submission whose provenance could not be established at all — either the
 * client sent no `extraction_model` stamp, or it sent one the server
 * normalized to `provider:'unknown'` (server.js's normalizeExtractionModel
 * maps a malformed/unparseable stamp here too, rather than to null). Kept
 * separate from HOLD_UNCALIBRATED_PROVIDER, which names a REAL, known
 * provider that simply hasn't been calibrated yet (codex-cli, byo-key) —
 * "we know who ran this and haven't blessed them" is a different fact than
 * "we don't know who ran this at all", and operators reading review_reason
 * should be able to tell the two apart.
 */
const HOLD_UNKNOWN_EXTRACTION_PROVIDER = 'unknown_extraction_provider';
 ...
 *
 * PART C: `extractionModel` (the /learn body's optional stamp) gates FIRST,
 * unconditionally, ahead of flag/consent/quality — a present-but-uncalibrated
 * provider always holds, full stop.
 *
 * EXTRACTION-MODEL-PROVENANCE (PUNCH-LIST P1) changed the MISSING case. It
 * used to be treated as calibrated-equivalent — fell through to the
 * flag/consent/quality checks unchanged, exactly like a genuine claude-code
 * stamp — which was a live fail-open: every shipped client stamps
 * `extraction_model` now (scripts/providers/index.js's runModel() makes
 * `identity` mandatory on every result it returns, deriving one centrally
 * when a provider module forgot), so a MISSING stamp today means either a
 * client old enough to predate the field, or one whose stamp got dropped —
 * neither of which this gate should treat as "known to be claude-code
 * quality-calibrated." A missing stamp now holds under the SAME distinct
 * reason as an explicit `provider:'unknown'` stamp (HOLD_UNKNOWN_EXTRACTION_
 * PROVIDER) rather than auto-publishing. Consequence, stated plainly: a
 * learning from a client too old (or too broken) to stamp its provider now
 * goes to pending_review instead of auto-publishing — nothing is refused or
 * lost, it costs one operator click instead of sailing through unverified.
 *
 * @param {object} params
 ...
 * @param {{provider?:string}|null} [params.extractionModel] the /learn body's
 *   extraction_model, or absent/null
 * @returns {{decision:'auto_publish', consent_version:string, min_quality:number}
 *          |{decision:'hold', reason:string, min_quality?:number}}
 */
function evaluateExtractionPublish({ flagEnabled, consentState, qualityTotal, accountSuspended, extractionModel } = {}) {
  const provider = extractionModel && typeof extractionModel === 'object' ? extractionModel.provider : undefined;
  // Missing stamp, or a provider that normalized to 'unknown' (server.js's
  // normalizeExtractionModel maps a malformed/unparseable stamp here, never
  // to null) — provenance could not be established at all. Hold, same
  // reason either way; see the docblock above for the consequence.
  if (provider === undefined || provider === null || provider === 'unknown' || typeof provider !== 'string' || !provider) {
    return { decision: 'hold', reason: HOLD_UNKNOWN_EXTRACTION_PROVIDER };
  }
  if (!CLEAN_LANE_CALIBRATED_PROVIDERS.includes(provider)) {
    return { decision: 'hold', reason: HOLD_UNCALIBRATED_PROVIDER };
  }
...
module.exports = {
  ...
  HOLD_UNCALIBRATED_PROVIDER,
  HOLD_UNKNOWN_EXTRACTION_PROVIDER,
  CLEAN_LANE_CALIBRATED_PROVIDERS,
