 ...
 * provider always holds, full stop. A MISSING extractionModel (pre-stamp
 * clients, or the direct-submission channel — this function is only ever
 * called inside the server.js submissionChannel==='extraction' guard, so
 * "direct" traffic never reaches it, but a not-yet-upgraded extraction
 * client can) is treated as calibrated-equivalent: it falls through to the
 * existing checks unchanged, so every submission that predates this stamp
 * behaves byte-identically to before this part shipped.
 *
 * @param {object} params
...
function evaluateExtractionPublish({ flagEnabled, consentState, qualityTotal, accountSuspended, extractionModel } = {}) {
  ...
  if (typeof provider === 'string' && !CLEAN_LANE_CALIBRATED_PROVIDERS.includes(provider)) {
