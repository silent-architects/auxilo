...
// CI-5 (PUNCH-LIST §30, 2026-07-19): TECHNICAL-ONLY scope — `communication`
// and `content-generation` are RETIRED learning categories (see
// lib/category-scope-migration.js, the server-side source of truth; the
// ci5-scope-enforcement test pins the copies equal).
...
- NON-TECHNICAL content of any kind (HARD SCOPE RULE — technical learnings only): interpersonal or communication strategy, copywriting/content/marketing insights, business or negotiation strategy, personal matters, creative-writing technique. Drop such candidates entirely; do not relabel them. Technical learnings about messaging/email/notification APIs belong under web-interaction or code-execution; content/data pipeline TECH belongs under data-processing.
- Anything failing the SYSTEM-FACT TEST (CI-7): extract ONLY when a system and a symptom are at the core — an error, an undocumented limitation, a reproducible behavior of an external tool/API/OS. If the candidate is advice about how to work (process, workflow, methodology, decision practice), do NOT extract it.

VALID CATEGORIES: data-processing, web-interaction, code-execution, storage-state, payment-financial, monitoring
