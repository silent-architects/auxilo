...
//
// Wave 2b (Wave-1 reviewer carry-in F4): writes serialize under a STORE-level
// promise mutex. The reviewer sketched an account-keyed mutex, but the ledger
// is ONE shared JSON file — under any future async I/O, account-keyed locking
// would let two accounts' whole-file read-modify-writes interleave and
// lost-update each other, so file-level is the sound version of the same
// intent (identical cost at current volume). If/when the ledger is compacted
// or sharded per account (FUTURE: the file grows one line per unique
// (account, learning) pair forever — shard by account before it matters),
// the shard key becomes the lock key.

const fs = require('fs');
const path = require('path');

// Store-level write mutex — same promise-chaining idiom as lib/earnings-lock.js.
 ...
 *
 * Async since Wave 2b: the load-modify-save runs under the store-level write
 * mutex so concurrent recordings can never lost-update the shared file. Call
 * sites are fire-and-forget best-effort (`.catch` logging) — a ledger write
 * must never block or fail a paid delivery.
 * @returns {Promise<void>}
 */
function recordPurchase(accountId, learningId, now = Date.now()) {
    if (!accountId || !learningId) return Promise.resolve();
    const op = _writeChain.then(() => {
    ...
    // Swallow errors in the chain so one failed write can't wedge all future
    // writes; the caller's returned promise still rejects for its own logging.
