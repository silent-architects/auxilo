...
async function verifyPaymentLocally(paymentHeader, expectedAmountUSD, recipientAddress, usdcAddress) {
    ...
    try {
        ...
        // 6. Verify USDC transfer(s) in logs.
        // M-6: SUM across ALL matching Transfer(from, to, value) logs where
        // to = recipientAddress (previously only the FIRST match was counted,
        // so split/multi-transfer payments under-counted). Validate each log's
        // data word is a well-formed 32-byte (66-char) hex word before BigInt().
        ...
        for (const log of receipt.logs) {
            ...
            if (typeof log.topics[2] !== 'string' || log.topics[2].length < 66) continue;
            // topics[2] = to address (padded to 32 bytes) — slice leading zeros
            const toAddr = '0x' + log.topics[2].slice(26).toLowerCase();
            if (toAddr !== recipientLower) continue;

            // M-6: reject malformed value words instead of silently mis-parsing.
            if (typeof log.data !== 'string' || !WORD_RE.test(log.data)) {
                setCache(cacheKey, false);
                return { valid: false, error: 'Malformed Transfer log value' };
            }

            sawTransfer = true;
            totalMicro += BigInt(log.data); // exact micro-USDC, no precision loss
        }
        ...
        // 7. Verify amount in exact BigInt micro-USDC (USDC has 6 decimals).
        // M-6: compare integers end-to-end so amounts above 2^53 cannot overflow.
        const expectedMicro = BigInt(Math.round(expectedAmountUSD * 1e6));
        if (totalMicro < expectedMicro) {
            setCache(cacheKey, false);
            return {
                valid: false,
                error: `Insufficient payment: ${totalMicro} < ${expectedMicro} (micro-USDC)`
            };
        }
