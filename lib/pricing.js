...
/**
 * CH-6 (2026-07-19): DISTINCT raters, not rating events. The >=3 activation
 * gates (community-quality blend below + calculateRatingMultiplier) previously
 * counted rating EVENTS — one account re-rating on the 1h cooldown could reach
 * 3 alone and flip both a 0.70-1.10 price multiplier and the 60%-weight
 * community blend (which, post-CH-4, flows into base_price via the daily cron).
 *
 * With the CH-6 rating store, quality.rater_scores is a per-account map
 * (replace-on-re-rate — one slot per account per learning), so its size is the
 * provably-distinct rater count. Pre-CH-6 legacy aggregates (quality.ratings
 * without a map) fall back to the event count: those shapes are FROZEN — any
 * new rating migrates them (server /rate lazy migration) and legacy events
 * then contribute zero to the gate — so the fallback cannot be grown by a
 * sybil.
 */
function distinctRaterCount(quality) {
  if (!quality) return 0;
  if (quality.rater_scores && typeof quality.rater_scores === 'object') {
    return Object.keys(quality.rater_scores).length;
  }
  return quality.ratings || 0; // frozen pre-CH-6 shapes only
}
...
function calculateQualityMultiplier(learning) {
  ...
  // CH-6: the blend activates on DISTINCT raters, not rating events.
  const hasRatings = distinctRaterCount(learning.quality) >= 3;
...
function calculateDemandMultiplier(learning, catalog) {
  ...
  if (catalog && catalog.length > 0) {
    const inCategory = catalog.filter(l =>
      isMarketVisible(l) && l.category === category && l.demand);
...
function calculateRatingMultiplier(learning) {
  // CH-6: activation counts DISTINCT raters (see distinctRaterCount above).
  if (!learning.quality || distinctRaterCount(learning.quality) < 3) return 1.0;
...
// ─── Verdict (V2 Section 7B — CH-5 rebalance 2026-07-19) ─────────────────────

/**
 * CH-5: the value benchmark the verdict compares price against.
 *
 * The old denominator was RAW DIY cost, so for a formula-priced item the ratio
 * decomposed as uniqueness × quality × demand × freshness — quality and
 * freshness raised the price AND were treated as pure overpricing. At the
 * ratified 1.5 uniqueness cap and cold demand (0.8412), any self-score total
 * > ~13.1/20 landed "expensive" — and the extraction floor is 14, so 100% of
 * approved facts were born expensive; the first-30-day freshness premium made
 * even no-QA items expensive on arrival (CH-4 Gate-A finding, spec §2.5).
 *
 * Fix: quality and freshness scale the VALUE a buyer receives exactly as they
 * scale the price (a higher-quality learning delivers proportionally more than
 * the raw DIY estimate; a fresher fact about a current API is worth more), so
 * they belong in the benchmark and cancel out of the overpricing test. What
 * remains in the ratio is uniqueness × demand:
 *
 *   - uniqueness stays OUT of the benchmark deliberately — the premium is
 *     price risk the verdict must keep reporting. This is what keeps the
 *     pre-CH-4 ungated 3.0x shelf reading "expensive" (ratio ≈ 2.52 cold) —
 *     the rebalance must not re-whitewash that inflation.
 *   - demand stays OUT — a demand-surged price above the benchmark is honest
 *     surge signal.
 *
 * Result at the 1.5 sparse cap, cold demand: ratio = 1.5 × 0.8412 = 1.262 →
 * "consider" for EVERY quality tier and EVERY freshness bracket — a fresh
 * q16-19 wall-test fact is approvable on arrival, while yesterday's ungated
 * shelf stays flagged. Spec: BUILD-SPEC-WAVE5A-2026-07-19.md §1.
 *
 * Community override: once >=3 DISTINCT raters exist (CH-6), the quality
 * multiplier blends 60% community — bad ratings SHRINK the benchmark, so a
 * poorly-rated item's verdict correctly degrades toward "expensive" even
 * though its price also drops (price falls at most 30% via the rating
 * multiplier; the benchmark can fall further).
 *
 * @param {object} learning
 * @returns {number} value benchmark in USD (> 0 for a known category)
 */
function estimateValueBenchmark(learning) {
  return estimateDiyCost(learning)
    * calculateQualityMultiplier(learning)
    * calculateFreshnessMultiplier(learning);
}

/**
 * Calculate the buyer verdict for a learning.
 * Compares current_price to the value-adjusted DIY benchmark (CH-5).
 *
 * @param {object} learning
 * @returns {'strong_buy' | 'recommended' | 'consider' | 'expensive'}
 */
function calculateVerdict(learning) {
  const benchmark = estimateValueBenchmark(learning);
  ...
  const ratio = currentPrice / benchmark;

  if (ratio < 0.3)  return 'strong_buy';   // Less than 30% of value benchmark — obvious win
  if (ratio < 0.75) return 'recommended';  // Less than 75% — clear value
  if (ratio < 1.5)  return 'consider';     // Within 1.5x — borderline
  return 'expensive';                       // More than 1.5x value benchmark — overprice signal
}
...
module.exports = {
  ...
  estimateDiyCost,
  estimateValueBenchmark,  // CH-5 (2026-07-19)
  distinctRaterCount,      // CH-6 (2026-07-19)
  qualityScore01,
