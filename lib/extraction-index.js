...
const { findNearDuplicate } = require('./similarity.js');
...
function buildPromptMemory(rows, opts = {}) {
  ...
  if (!eligible.length || maxRows === 0 || estimatePromptTokens(MEMORY_INSTRUCTIONS) > maxTokens) {
    return { section: '', estimated_tokens: 0, included_count: 0, category_hints: [...hints] };
  }

  const lines = [];
  for (const row of eligible) {
    if (lines.length >= maxRows) break;
    const candidateLines = [...lines, promptMemoryLine(row)];
  ...
  try {
    ...
    for (const candidate of input) {
      ...
      if (result.verdict === 'flag') {
        dropped.push({ candidate, match: result.match });
      } else {
        kept.push(candidate);
      }
    }
