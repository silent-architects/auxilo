...
// Sweep expired windows so the store cannot grow unbounded on a public
// endpoint (one entry per distinct IP otherwise persists forever). unref()
// keeps the interval from holding the process open in tests.
const waitlistRateSweep = setInterval(() => {
  const cutoff = Date.now() - WAITLIST_RATE_WINDOW_MS;
  for (const [ip, times] of waitlistRateStore) {
    const live = times.filter(t => t > cutoff);
    if (live.length === 0) waitlistRateStore.delete(ip);
    else if (live.length !== times.length) waitlistRateStore.set(ip, live);
  }
}, WAITLIST_RATE_WINDOW_MS);
if (waitlistRateSweep.unref) waitlistRateSweep.unref();
...
function removeWaitlistEmail(rawEmail) {
  const email = normalizeEmail(rawEmail);
  if (!email) return 0;
  const list = loadWaitlist();
  const kept = list.filter((entry) => !entry || entry.email !== email);
  const removed = list.length - kept.length;
  if (removed > 0) saveWaitlist(kept);
  return removed;
}
