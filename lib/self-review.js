...
function applySelfDecision(learnings, accountId, id, decision, opts = {}) {
  ...
  // accounts retain the pre-R13 self-review flow byte-for-byte. This guard is
  // shared by single + bulk decisions.
  if (decision === 'approve') {
    ...
    // establish it. Missing account context fails closed.
    if (!isPublicationTrusted(opts.account)) {
