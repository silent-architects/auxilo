...
const { TECH_LEARNING_CATEGORIES } = require('./category-scope-migration.js');
const {
  buildAccountVocabulary,
  parseCommonDevTerms,
} = require('./account-vocab.js');
const { isPublicationTrusted } = require('./publication-authority.js');

const ACCOUNT_VOCAB_SIGNAL = 'account_vocab';
const NON_BYPASSABLE_PLATFORM_HOLD_REASONS = new Set([
  'report_auto_hide',
  'malicious_content',
]);

function accountVocabEvidence(matches) {
  if (!Array.isArray(matches) || matches.length === 0) return [];
  const term = String(matches[0].term || '').slice(0, 60);
  if (!term) return [];
  const hint =
    `recurring term '${term}' appears only in your own learnings — ` +
    'likely internal system vocabulary; genericize via sanitize';
  return [{ signal: ACCOUNT_VOCAB_SIGNAL, excerpt: term, hint }];
}
...
function projectPending(l) {
  ...
  if (l.malicious_verdict) out.malicious_verdict = l.malicious_verdict;
  if (l.malicious_reason) out.malicious_reason = l.malicious_reason;
  if (Array.isArray(l.platform_hold_reasons) && l.platform_hold_reasons.length > 0) {
    out.platform_hold_reasons = l.platform_hold_reasons.slice();
  }
  if (l.possible_duplicate_of) {
    ...
    if (l.near_duplicate_why) out.near_duplicate_why = l.near_duplicate_why;
    if (l.near_duplicate_evidence) {
      out.near_duplicate_evidence = l.near_duplicate_evidence;
    }
  }
  return out;
}
 ...
 * @param {'approve'|'reject'|'keep_private'} decision
...
function applySelfDecision(learnings, accountId, id, decision, opts = {}) {
  if (decision !== 'approve' && decision !== 'reject' && decision !== 'keep_private') {
    return { ok: false, code: 'bad_decision', status: 400, error: 'decision must be "approve", "reject", or "keep_private"' };
  }
  if (!accountId) {
  ...
  if (decision === 'approve' && learning.visibility === 'private') {
    return {
      ok: false,
  ...
  if (decision === 'approve' && !TECH_LEARNING_CATEGORIES.includes(learning.category)) {
    return {
      ok: false,
      code: 'category_out_of_scope',
      status: 409,
      error: `Category '${learning.category}' is out of scope — Auxilo publishes technical learnings only. ` +
        `Resubmit the content under one of: ${TECH_LEARNING_CATEGORIES.join(', ')}, or reject this item.`,
    };
  }

  // R13: contributor consent is not publication authority. Until an operator
  // grants trust (directly or through prior admin approval), contributor
  // self-approval cannot publish: platform-flagged items require operator eyes,
  // and even a clean first public learning requires operator approval. Trusted
  // accounts retain the pre-R13 self-review flow byte-for-byte except that a
  // report auto-hide or malicious-content platform hold always requires
  // operator review. This guard is shared by single + bulk decisions.
  if (decision === 'approve') {
    // R13 R2/R3-B: trust comes only from a durable operator grant or prior
    // admin approval. Wallet verification and self-review provenance never
    // establish it. Missing account context fails closed. Report auto-hides and
    // malicious-content holds are operator controls, not contributor-consent
    // holds, so publication trust cannot bypass them.
    const trustCannotClearPlatformHold =
      Array.isArray(learning.platform_hold_reasons) &&
      learning.platform_hold_reasons.some((reason) =>
        NON_BYPASSABLE_PLATFORM_HOLD_REASONS.has(reason));
    if (trustCannotClearPlatformHold || !isPublicationTrusted(opts.account)) {
      const flags = screenFlags(learning, { accountVocabMatches: opts.accountVocabMatches });
      if (trustCannotClearPlatformHold || flags.length > 0) {
        return {
          ok: false,
          code: 'platform_review_required',
          status: 409,
          error: 'This learning is held by an Auxilo platform screen or policy and requires operator review.',
          flags,
        };
      }
      return {
        ...
        status: 409,
        error: 'Your first public learning requires Auxilo operator approval before contributor self-publication is enabled.',
      };
    }
  }
  ...
  } else if (decision === 'keep_private') {
    learning.status = 'approved';
    learning.visibility = 'private';
    delete learning.unlock_price;
    delete learning.pricing;
    delete learning.demand;
    delete learning.earnings;
    learning.moderation = 'manual';
    learning.self_review_action = { action: 'self_keep_private', by: accountId, at: now };
  } else {
 ...
 *
 * @param {object} l
 * @returns {string[]} persisted platform screen/policy flags
 */
function screenFlags(l, opts = {}) {
  const flags = [];
  ...
  if (l.malicious_verdict && l.malicious_verdict !== 'none') flags.push('malicious');
  if (Array.isArray(l.platform_hold_reasons) && l.platform_hold_reasons.length > 0) {
    flags.push('platform_hold');
  }
  if (Array.isArray(opts.accountVocabMatches) && opts.accountVocabMatches.length > 0) {
    flags.push(ACCOUNT_VOCAB_SIGNAL);
  }
  return flags;
}
 ...
 * @returns {string|null}
 */
function deriveWhy(l, flags, opts = {}) {
  if (!Array.isArray(flags) || flags.length === 0) return null;
  if (flags.includes(ACCOUNT_VOCAB_SIGNAL)) {
    const evidence = accountVocabEvidence(opts.accountVocabMatches);
    if (evidence[0]) return evidence[0].hint;
  }
  ...
  if (flags.includes('near_duplicate') && l.possible_duplicate_of) {
    if (typeof l.near_duplicate_why === 'string' && l.near_duplicate_why) {
      return l.near_duplicate_why;
    }
  ...
  if (flags.includes('malicious')) {
    return typeof l.malicious_reason === 'string' && l.malicious_reason
      ? l.malicious_reason
      : `The malicious-content screen held this (${l.malicious_verdict || 'unspecified'}).`;
  }
  if (flags.includes('platform_hold')) {
    const reasons = Array.isArray(l.platform_hold_reasons)
      ? l.platform_hold_reasons.join(', ')
      : 'unspecified';
    return `Held by Auxilo platform policy (${reasons}); operator review is required.`;
  }
  return null;
}
 ...
 * @returns {object}
 */
function projectTriageRow(l, opts = {}) {
  const flags = screenFlags(l, opts);
  const q = qualityOf(l);
  const row = {
    id: l.id,
    title: l.title,
  ...
  const why = deriveWhy(l, flags, opts);
  if (why) row.why = why;
  ...
  const signalNames = Array.isArray(l.sensitivity_signals)
    ? [...l.sensitivity_signals]
    : [];
  if (flags.includes(ACCOUNT_VOCAB_SIGNAL)) signalNames.push(ACCOUNT_VOCAB_SIGNAL);
  if (signalNames.length > 0) {
    row.sensitivity_signals = [...new Set(signalNames)];
  }
  const vocabEvidence = accountVocabEvidence(opts.accountVocabMatches);
  if (vocabEvidence.length > 0) {
    row.sensitivity_evidence = vocabEvidence;
  }
  ...
  if (l.possible_duplicate_of) {
    ...
    if (l.near_duplicate_why) row.near_duplicate_why = l.near_duplicate_why;
    if (l.near_duplicate_evidence) {
      row.near_duplicate_evidence = l.near_duplicate_evidence;
    }
  }
  return row;
}
...
function summarizeOwnPending(learnings, accountId, opts = {}) {
  ...
  let accountVocabMatches = {};
  if (opts.accountVocab && own.length > 0) {
    accountVocabMatches = buildAccountVocabulary(learnings, opts.accountVocab).matches_by_learning_id;
  }
  const items = own.map((learning) => projectTriageRow(learning, {
    accountVocabMatches: accountVocabMatches[learning.id] || [],
  }));
  items.sort((a, b) => {
  ...
  const byScreen = {
    injection: 0,
    content_sensitivity: 0,
    near_duplicate: 0,
    process_advice: 0,
    [ACCOUNT_VOCAB_SIGNAL]: 0,
    malicious: 0,
    platform_hold: 0,
  };
  ...
  for (const row of items) {
    ...
    for (const f of row.flags) byScreen[f] = (byScreen[f] || 0) + 1;
  ...
  if (opts.signal && typeof opts.signal === 'string') {
    ...
    rows = rows.filter((r) =>
      ...
      (sig === ACCOUNT_VOCAB_SIGNAL && r.flags.includes(ACCOUNT_VOCAB_SIGNAL)) ||
 ...
 * flags; account_vocab is derived from the full corpus when its review-time
 * options are supplied; anything else matches sensitivity_signals membership.
 * Ownership is filtered like every other path in this module. Pure — the
 * server's counted reject-by-signal gate compares expected_count against this
 * selection.
 *
 * @param {Array<object>} learnings
 * @param {string} accountId
 * @param {string} signal
 * @param {object} [opts] { accountVocab: { config, commonDevTerms } }
 * @returns {string[]} ids (stable catalog order)
 */
function selectPendingIdsBySignal(learnings, accountId, signal, opts = {}) {
  if (!Array.isArray(learnings) || !accountId || typeof signal !== 'string' || !signal) return [];
  // SPEC3-F1 rider: account_vocab is a review-time derivation, not a persisted
  // sensitivity signal. Build it from the same full-corpus options used by the
  // summary so the counted selector cannot disagree with counts.by_signal.
  const accountVocabMatches = signal === ACCOUNT_VOCAB_SIGNAL && opts.accountVocab
    ? buildAccountVocabulary(learnings, opts.accountVocab).matches_by_learning_id
    : {};
  const out = [];
  for (const l of learnings) {
    if (!l || l.status !== PENDING_STATUS || l.contributor_account_id !== accountId) continue;
    const flags = screenFlags(l, {
      accountVocabMatches: accountVocabMatches[l.id] || [],
    });
    const matches =
      ...
      (signal === ACCOUNT_VOCAB_SIGNAL && flags.includes(ACCOUNT_VOCAB_SIGNAL)) ||
 ...
 * @param {Array<object>} learnings
 * @param {string} accountId
 * @param {Array<{id:string, decision:'approve'|'reject'|'keep_private', reason?:string}>} decisions
...
function applyBulkDecisions(learnings, accountId, decisions, opts = {}) {
  ...
  const results = [];
  const counts = { processed: 0, approved: 0, kept_private: 0, rejected: 0, failed: 0, idempotent: 0, changed: 0 };
  ...
  decisions.forEach((entry, index) => {
    ...
    if (decision !== 'approve' && decision !== 'reject' && decision !== 'keep_private') {
      counts.failed += 1;
      results.push({ ...base, ok: false, code: 'bad_decision', error: 'decision must be "approve", "reject", or "keep_private"' });
      return;
    }
    ...
    const targetStatus = decision === 'reject' ? 'rejected' : 'approved';
    const targetVisibility = decision === 'keep_private' ? 'private'
      : decision === 'approve' ? 'public' : null;
    const currentVisibility = learning && learning.visibility === 'private' ? 'private' : 'public';
    if (learning && learning.contributor_account_id === accountId &&
        learning.status === targetStatus &&
        (targetVisibility === null || currentVisibility === targetVisibility)) {
      counts.idempotent += 1;
    ...
    const applied = applySelfDecision(learnings, accountId, id, decision, {
      ...
      accountVocabMatches: opts.accountVocabMatches,
    });
    if (!applied.ok) {
      counts.failed += 1;
    ...
    else if (decision === 'keep_private') counts.kept_private += 1;
