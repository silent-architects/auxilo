...
function loadModelConfig(configPath) {
  ...
  try {
    ...
    console.error(`[sensitivity-llm] model_config.json unreadable at ${configPath} (${err.code || err.message}) — using hardcoded default model ${DEFAULT_MODEL}`);
    parsed = null;
  }
...
/** Test hook: drop the cached parse so a rewritten file is re-read. */
 ...
 * limited), any 5xx, or a timeout (AbortError). Everything else — auth 401/403,
 * 400 bad request, network failure, parse errors — is NOT retried: the caller
 * fails closed immediately, exactly as before.
 *
 * @param {Error} err
 * @returns {boolean}
 */
function isRetryableProviderError(err) {
  if (!err) return false;
  if (err.name === 'AbortError' || /abort/i.test(err.message || '')) return true;
  ...
  try {
    for (let i = 0; i < chain.length; i++) {
      try {
        ...
        // SCREEN-MODEL-CONFIG: a retryable provider error (404/429/5xx/timeout)
        // with a fallback model left in the chain → try the next model once.
        // Anything else, or an exhausted chain, rethrows into the UNCHANGED
        // fail-closed path below (same shape, same reason fields).
