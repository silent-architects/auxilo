        ...
        line_items: [{
            price_data: {
                ...
                product_data: {
                    ...
                    // own hosted page or receipt (D11).
                    // CREDITS-QUERIES-RESIDUAL: the pack_queries metadata field
                    // (metadata → webhook → purchases.jsonl) that D11 left in place
                    // is now retired along with PACKS[*].queries above — packs grant
                    // unlocks only. Older sessions in Stripe's queue may still carry
                    // a stale pack_queries key; the webhook no longer reads it.
