...
function rotateKeyEntry(account, targetKey) {
    ...
    // D2-F2: prune rotation debris beyond the per-label ceiling. The new entry
    // was pushed LAST and compaction preserves order, so it stays last —
    // callers computing `api_keys.length - 1` for the index remain correct.
    const { removed } = compactRotatedKeys(account);
    return { rawKey, entry, oldHash: targetKey.hash, compacted: removed };
}
