...
// never gated on payee-agency — when there is no external Builder to receive a share.
// That is true when the learning carries no external contributor_account_id, OR its
// contributor wallet is the platform's own wallet. Only a learning with a real external
// contributor_account_id whose wallet is NOT the platform wallet represents third-party
// funds; those are the ones the unlock handler must refuse until the Builder's §5.10
// payee-agency is in force. `platformWallet` is passed in (defined in server.js) so this
// stays a pure predicate. Comparison is case-insensitive (checksummed vs lowercase addrs).
function isPlatformContributor(learning, platformWallet) {
    if (!learning) return true;
    const platform = platformWallet ? String(platformWallet).toLowerCase() : null;
    const cw = learning.contributor_wallet ? String(learning.contributor_wallet).toLowerCase() : null;
    if (cw && platform && cw === platform) return true;       // contributor IS the platform wallet
    if (!learning.contributor_account_id) return true;         // no external Builder account attached
    return false;                                              // real external Builder contributor
}
...
/** Key scope ordering for minimum-scope checks (GOV-3 review M-1). */
...
function requireSessionOrApiKey(minScope = 'contribute') {
    ...
    return async function sessionOrApiKeyAuth(c, next) {
        ...
        if (!accountId) {
            ...
            const rank = SCOPE_RANK[keyResult.scope] || 0;
            if (rank < minRank) {
...
async function resolveAccountFromRequest(c, minScope = 'read') {
    ...
    if (bearer) {
        const payload = await verifyJwt(bearer);
        if (payload && payload.accountId) accountId = payload.accountId;
    }
...
function setupAccountRoutes(app) {
    ...
    app.post('/account/api-keys', requireAuth, async (c) => {
        ...
        if (hasLabel || !hasName) {
            ...
            // unless the caller explicitly opts into a higher scope. An invalid
            // scope is rejected rather than silently upgraded.
            const LABELED_SCOPES = ['read', 'contribute', 'admin'];
