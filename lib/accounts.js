...
// (no non-platform wallet AND no account). `platformWallet` is passed in (defined in
// server.js) so this stays a pure predicate. Comparison is case-insensitive.
function isPlatformContributor(learning, platformWallet) {
    ...
    // (a) a non-platform contributor wallet is an identifiable external payee -> NOT platform.
    if (cw && (!platform || cw !== platform)) return false;
    ...
    // Otherwise: contributor is the platform wallet, or there is no external payee at all.
    return true;
}
