...
// Wave 3.4: env-overridable for TEST isolation only (mirrors the shipped
// AUXILO_IDENTITY_FILE / AUXILO_PURCHASE_LEDGER_FILE idiom) — two test files
// fixturing the live data/accounts.json from concurrent node --test processes
// race each other's backup/restore. Prod never sets this; absent = unchanged.
const ACCOUNTS_FILE = process.env.AUXILO_ACCOUNTS_FILE || path.join(DATA_DIR, 'accounts.json');
const MAGIC_LINKS_FILE = process.env.AUXILO_MAGIC_LINKS_FILE || path.join(DATA_DIR, 'magic_links.json');
...
// First-class account ids that identify Auxilo itself rather than an external
// Builder. This mirrors the platform-wallet identity used by the unlock gate.
const PLATFORM_ACCOUNT_IDS = new Set(['acc_platform']);
const PLATFORM_ACCOUNT_EMAIL_TO_ID = new Map([
    ['platform@auxilo.io', 'acc_platform'],
]);

// SEED-ATTR: account attribution is catalog metadata for platform-owned rows,
// never a contributor-ledger identity. Every money path calls this one guard
// before resolving, moving, or replaying earnings.
function normalizeCreditingContributorAccountId(accountId) {
    return PLATFORM_ACCOUNT_IDS.has(accountId) ? null : (accountId || null);
}

// REV 2c: reserve the canonical platform mailbox even before the offline
// migration creates its account record, while also refusing any later alias
// attached to a registered platform account id.
function platformAccountIdForEmail(email) {
    const normalizedEmail = String(email || '').trim().toLowerCase();
    const reservedId = PLATFORM_ACCOUNT_EMAIL_TO_ID.get(normalizedEmail);
    if (PLATFORM_ACCOUNT_IDS.has(reservedId)) return reservedId;

    const match = Object.entries(loadAccounts()).find(([accountId, account]) =>
        PLATFORM_ACCOUNT_IDS.has(accountId) &&
...
// wallet, or (b) a contributor_account_id that is not a registered platform identity.
// Either one means an unlock would send a share to a third party, which the handler must
// refuse until that Builder's §5.10 payee-agency is in force. A missing account_id is NOT
// sufficient to call a learning platform-owned:
...
// Platform-owned = no non-platform wallet AND either a registered platform account or no
// account at all. `platformWallets` is passed in (defined in server.js) so this stays a pure
// predicate; it accepts one wallet OR an array because
...
function isPlatformContributor(learning, platformWallets) {
    ...
    if (PLATFORM_ACCOUNT_IDS.has(learning.contributor_account_id)) return true;
...
function issuePurposeMagicLink(email, purpose) {
...
function consumePurposeMagicLink(rawToken, purpose) {
    if (!rawToken || typeof rawToken !== 'string' || rawToken.trim().length !== 43 || !purpose) return null;
    const tokenHash = crypto.createHash('sha256').update(rawToken.trim()).digest('hex');
    const links = loadMagicLinks();
    const entry = links[tokenHash];
    if (!entry || entry.purpose !== purpose) return null;
    if (entry.expires_at < Date.now()) {
        delete links[tokenHash];
        saveMagicLinks(links);
        return null;
    }
    delete links[tokenHash];
...
function removeMagicLinksForEmail(email) {
    if (!email) return 0;
    const links = loadMagicLinks();
    let removed = 0;
    for (const [hash, entry] of Object.entries(links)) {
        if (entry && entry.email === email) {
            delete links[hash];
            removed++;
        }
    }
    if (removed > 0) saveMagicLinks(links);
    return removed;
}

function countMagicLinksForEmail(email) {
    if (!email) return 0;
    return Object.values(loadMagicLinks()).filter((entry) => entry && entry.email === email).length;
}
...
// Both login and destructive email links share this limiter. The call order is
// intentionally identical to POST /auth/magic-link, including its accounting
// behavior when an IP limit is reached after an email slot was consumed.
function isMagicLinkRateLimited(email, ip) {
    return isRateLimited('email', email, RATE_LIMIT_BY_EMAIL)
        || isRateLimited('ip', ip, RATE_LIMIT_BY_IP);
}
 ...
 */
// D2-F2: rotation-debris ceiling. Rotation deactivates the old entry in place
// (audit trail), so a rotate LOOP on a stolen key grew accounts.json without
// bound — and that file is parsed on every authenticated request (DoS
// amplifier). Keep the newest N rotated-out entries per label; prune older.
const ROTATED_INACTIVE_KEEP = 3;

/**
 * D2-F2: compact rotation debris on one account (pure mutation — caller
 * persists). Removes INACTIVE entries carrying `rotated_at` beyond the newest
 * ROTATED_INACTIVE_KEEP per label. Never touches active entries, and never
 * touches inactive entries WITHOUT rotated_at (revocations keep their audit
 * rows). Order of surviving entries preserved.
 *
 * CALLER NOTE: removal shifts api_keys array positions, and the in-memory
 * index stores key_index for last_used_at updates — when this returns
 * removed > 0 the caller must rebuildKeyIndex() after persisting.
 *
 * @param {object} account  Account object (api_keys already migrated).
 * @returns {{ removed: number }}
 */
function compactRotatedKeys(account) {
    const keys = account.api_keys || [];
    // Newest-first rotated-debris ids per label.
    const byLabel = new Map();
    for (const k of keys) {
        if (k.active !== false || !k.rotated_at) continue;
        const label = k.label || k.name || 'default';
        if (!byLabel.has(label)) byLabel.set(label, []);
        byLabel.get(label).push(k);
    }
    const drop = new Set();
    for (const list of byLabel.values()) {
        list.sort((a, b) => String(b.rotated_at).localeCompare(String(a.rotated_at)));
        for (const k of list.slice(ROTATED_INACTIVE_KEEP)) drop.add(k);
    }
    if (drop.size === 0) return { removed: 0 };
    account.api_keys = keys.filter((k) => !drop.has(k));
    return { removed: drop.size };
}

// Gate-A F5: global inactive-debris ceiling. The per-label rotated ceiling
// above bounds nothing when LABELS are attacker-chosen — a create→revoke loop
// with a fresh label each cycle leaves one permanent dead entry per cycle
// (revocations were kept forever "for audit"), and rotation debris likewise
// accumulates per unique label. This ceiling is label-agnostic: keep the
// newest INACTIVE_DEBRIS_KEEP dead entries per ACCOUNT (any kind), drop the
// rest. Chosen over a creation rate limit because a rate limit only SLOWS
// the same unbounded growth; the ceiling hard-bounds accounts.json at
// <= 10 active + INACTIVE_DEBRIS_KEEP dead entries per account regardless of
// label churn. 15 dead keys is an ample audit tail — deeper history lives in
// the server logs, which record every creation/rotation/revocation.
const INACTIVE_DEBRIS_KEEP = 15;

/** End-of-life stamp for ordering dead entries (newest kept). */
function keyDeathStamp(k) {
    const v = k.rotated_at || k.revoked_at || k.created_at || '';
    return typeof v === 'number' ? new Date(v).toISOString() : String(v);
}

/**
 * Gate-A F5: cap TOTAL inactive entries on one account at
 * INACTIVE_DEBRIS_KEEP (newest by death stamp; order preserved; active
 * entries never touched). Pure mutation — caller persists, and when
 * removed > 0 the caller must rebuildKeyIndex() (positions shift).
 *
 * @param {object} account  Account object (api_keys already migrated).
 * @returns {{ removed: number }}
 */
function capInactiveDebris(account) {
    const keys = account.api_keys || [];
    const inactive = keys.filter((k) => k.active === false);
    if (inactive.length <= INACTIVE_DEBRIS_KEEP) return { removed: 0 };
    const keep = new Set(
        inactive
            .slice()
            .sort((a, b) => keyDeathStamp(b).localeCompare(keyDeathStamp(a)))
            .slice(0, INACTIVE_DEBRIS_KEEP)
    );
    const before = keys.length;
    account.api_keys = keys.filter((k) => k.active !== false || keep.has(k));
    return { removed: before - account.api_keys.length };
}
...
function rotateKeyEntry(account, targetKey) {
    ...
    // D2-F2: prune rotation debris beyond the per-label ceiling, then the
    // F5 global inactive ceiling. The new entry was pushed LAST and both
    // compactions preserve order and never touch active entries, so it stays
    // last — callers computing `api_keys.length - 1` remain correct.
    const { removed } = compactRotatedKeys(account);
    const { removed: capped } = capInactiveDebris(account);
    return { rawKey, entry, oldHash: targetKey.hash, compacted: removed + capped };
}
...
function setupAccountRoutes(app) {
    ...
    app.post('/auth/magic-link', async (c) => {
        ...
        // SEED-ATTR REV 2c: platform accounts are non-login identities. Refuse
        // to generate or persist a redeemable token, but still traverse the
        // normal delivery wait below with a tokenless URL. An early return here
        // would make Resend latency an account-existence oracle even if the
        // response bytes matched.
        const platformAccountId = platformAccountIdForEmail(normalizedEmail);
        let rawToken = null;
        ...
        const verifyUrl = rawToken ? `${dashboardUrl}?token=${encodeURIComponent(rawToken)}` : dashboardUrl;
    ...
    app.get('/auth/verify', async (c) => {
        ...
        // Purpose-bound tokens are never redeemable as login tokens. Leave a
        // valid deletion token intact for its dedicated confirmation endpoint.
        if (matchedEntry.purpose) {
            return c.json({ error: 'Invalid or expired token' }, 401);
        }
        ...
        // SEED-ATTR REV 2c: consume but never redeem a token reserved for a
        // platform identity. Reuse the ordinary invalid-token response exactly.
        if (platformAccountIdForEmail(matchedEntry.email)) {
            return c.json({ error: 'Invalid or expired token' }, 401);
        }
    ...
    app.post('/account/api-keys', requireAuth, async (c) => {
        ...
        if (hasLabel || !hasName) {
            ...
            // D2-F2: counted on ACTIVE keys only (device-authorize already
            // does) — the old raw-length count included rotated-out/revoked
            // dead entries, so 10 rotations bricked labeled minting forever.
            if ((account.api_keys || []).filter(k => k.active !== false).length >= 10) {
        ...
        // D2-F2: the legacy path was fully UNCAPPED — same unbounded-growth
        // class the labeled cap exists to stop. Same active-only 10-key cap.
        if ((account.api_keys || []).filter(k => k.active !== false).length >= 10) {
            return c.json({ error: 'Maximum 10 API keys per account' }, 400);
        }
