...
// CP-6: the payee-agency (§5.10) is "in force" for a Builder once they have accepted a
// ToS version that CONTAINS §5.10 — the receipt-side gate for accrual keys on THIS, not
// hasAcceptedCurrentTos. The distinction matters: a later, UNRELATED ToS version bump must
// NOT re-quarantine a Builder whose already-accepted version established the agency.
// v1: CURRENT_TOS_VERSION ('2026-07-04-payee-agency-a1') is the FIRST assent-captured
// version and it introduces §5.10, so any recorded acceptance IS a §5.10 acceptance →
// (accepted_at && tos_version) is sufficient.
// TODO(version-bump): when a 2nd ToS version ships, gate on the accepted version being
// >= the §5.10-introducing version (a PAYEE_AGENCY_MIN_VERSION constant) so unrelated
// bumps don't re-quarantine already-covered Builders.
function isPayeeAgencyInForce(account) {
    return !!(account && account.accepted_at && account.tos_version);
}
