 ...
 * old/new file set, but `<bin>` itself is never missing. The CURRENT
 * process has already loaded/executed code from the OLD files and keeps
 * running on them for the rest of this invocation (Node does not hot-swap
 * already-`require()`d modules, and a live process reading files out from
 * under itself mid-run is exactly what the in-flight guard below exists to
 * prevent for OTHER concurrent processes). The new copy takes effect
...
/** @deprecated kept only as the legacy path name some callers/logs referenced;
 * prefer extractionLockDir / extractionMarkerPath. Not written to directly. */
...
function stageAndSwap(homeDir, extractedPackageDir, installer) {
  ...
  extractedInstaller.installRunnerAtomic(homeDir, { packageRoot: extractedPackageDir });
  return binRoot;
}
...
async function checkAndApplyRunnerUpdate(homeDir, opts = {}) {
  ...
  try {
    ...
    // or a prior update that never wrote one) must NOT fall through to
    // installing whatever the registry named. Refuse unless the fetched
    // version is newer than the RUNNING package's own version — i.e. we
    // only trust an unknown-installed state enough to catch up to
    // ourselves, never to blindly install an arbitrary registry answer. ──
    const installed = installer.installedRunnerVersion(homeDir);
    if (installed === null) {
