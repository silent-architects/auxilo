...
describe('WAVE-E3 items 3+4 (live): legal routes render the nav and no literal --- paragraphs', { timeout: 180_000 }, () => {
  ...
  it('GET /legal/supported-clients carries the D1-approved Claude Code drafting paragraph and drops "on your own subscription"', async (t) => {
    if (bootSkipReason) { t.skip(bootSkipReason); return; }
    const res = await fetch(`${baseUrl}/legal/supported-clients`);
    assert.equal(res.status, 200);
    const body = await res.text();
    assert.match(body, /Today the runner drafts learnings through the model client you are signed in to on your machine, Claude Code, for every client it captures\. Without one, captured sessions are held and nothing is submitted\. Or set a provider key of your own with auxilo provider set\. It stays on your machine and Auxilo never receives it\. Per-client model paths are being built\./);
    assert.doesNotMatch(body, /on your own subscription/);
  });

  it('GET /legal/supported-clients step 4 ("Local extraction") carries the D1-approved drafting sentence', async (t) => {
    if (bootSkipReason) { t.skip(bootSkipReason); return; }
    const res = await fetch(`${baseUrl}/legal/supported-clients`);
    assert.equal(res.status, 200);
    const body = await res.text();
    assert.match(body, /Local extraction<\/strong>\s*:\s*Claude Code, when you are signed in to it, or a provider key you set yourself, drafts learnings from the scrubbed text\./);
  });
