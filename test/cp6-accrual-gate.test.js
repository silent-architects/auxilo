 ...
 * (payee-agency receipt-side close; Gate-B Condition A / red-team P0-1)
 *
 * The payee-agency defense (Terms §5.10) turns on Auxilo's RECEIPT of the Builder
 * Share as the Builder's agent — the custodial accrual credit. Builder Share received
 * before a Builder has accepted §5.10 is HELD in `unassented_pending` (non-withdrawable —
 * getWithdrawableBalance reads pending_balance only) and moved into `pending_balance` the
 * moment they affirmatively accept. These tests prove the held-bucket data model + the
 * conversion helper (real logic) and that the accrual gate, WAL replay, and accept-terms
 * conversion are wired correctly (source-level, same style as tos-clickwrap-assent.test.js).
 *
