...
describe('Idempotency: checkIdempotency + recordExtraction', () => {
  ...
  it('/extract handler calls recordExtraction after processing', () => {
    const extractIdx = SERVER_SRC.indexOf("app.post('/extract'");
    // R13 adds publication-authority and malicious-verdict persistence ahead
    // of the unchanged recordExtraction tail; widen the structural read only.
    const handler = SERVER_SRC.slice(extractIdx, extractIdx + 28000);
