...
function passingResponses(overrides = {}) {
  return {
    ...
    [`npm view ${PACKAGE_NAME}@0.9.24 version`]: new Error('npm ERR! 404'),
    ...overrides,
  };
}
...
test('prepublish-guard: passes when HEAD == origin/main, tree clean, version unpublished', () => {
  ...
  assert.equal(result.version, '0.9.24');
});
...
test('prepublish-guard: refuses when the current version is already published', () => {
  const responses = passingResponses({
    [`npm view ${PACKAGE_NAME}@0.9.24 version`]: '0.9.24',
  });
  const result = check({ run: fakeRun(responses), env: {} });
  assert.equal(result.ok, false);
...
test('prepublish-guard: full OIDC skips only whoami and prints one secret-free explanation', () => {
  ...
  assert.deepEqual(calls, [
    ...
    'git status --porcelain',
    `npm view ${PACKAGE_NAME}@0.9.24 version`,
  ]);
...
test('prepublish-guard: OIDC still refuses an already published version', () => {
  const result = check({
    run: fakeRun(passingResponses({ [`npm view ${PACKAGE_NAME}@0.9.24 version`]: '0.9.24' })),
    env: OIDC_ENV,
    log: () => {},
  });
  assert.equal(result.ok, false);
