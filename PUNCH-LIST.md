...
| CP-5 | **Clickwrap assent capture (red-team P0-3)** — the payee-agency appointment (ToS §5.10) does not bind without affirmative, captured assent. Built: `CURRENT_TOS_VERSION` + consent artifact (`tos_version`/`accepted_at`/`accepted_ip`/`accepted_ua` + append-only log) in `lib/accounts.js`; `POST /account/accept-terms` + `GET /account/terms-status` (session OR API key); **blocking 403 `TERMS_NOT_ACCEPTED` gate at `POST /account/link-wallet` + both withdrawal rails**; web clickwrap (unchecked box + Accept) in `public/dashboard.html`; MCP handshake `auxilo_accept_terms` (V-3, explicit `agree`) in `mcp-server.js`; durable hash-chained record `lib/tos-acceptance-log.js` (parallel reuse session, wired in — write-side evidence only, never a gate). Tests: `test/tos-clickwrap-assent.test.js` (22/22; suite 595/602 w/ 7 pre-existing). Docs: UX-FLOWS §3 STEP 6.5 + flow 7 V-3, ONBOARDING-COPY §3.5. Gate A (eng+sec+QA) no blocker (2 LOW fixes applied: fail-closed /withdraw, idempotent accept); Gate B (contracts-counsel) ENFORCEABLE-WITH-CONDITIONS (web clickwrap HIGH; MCP human-principal binding reserved for counsel). | P0 | BUILD-1 + GOV-3 + BUILD-4 + SPEC-2/3 + GOV-2 | ✅ CODE + TESTS DONE (2026-07-04) — **DEPLOY-GATED: ships all-or-nothing with the §5.10 ToS text (must go live in the SAME deploy; neither alone). `CURRENT_TOS_VERSION` must match the §5.10 effective identifier in `docs/TERMS-OF-SERVICE.md` at ship. Hold for Tyler's ship word.** Merge of two sessions (task_3d4030a6 + task_a12543aa); reconcile/attribute at commit. Uncommitted in working tree. |
...
| AUD-A3 | Deploy-time constructor (usdc/feeWallet/settler) is the entire immutable trust root and is uncovered. | — (gate) | BUILD-1 | OPEN — deploy checklist: canonical-USDC assertion, fresh single-purpose feeWallet, bytecode/Etherscan verification, post-deploy immutable read-back before flipping the flag. |
...
| AUD-F6 | Client instruction "sign nonce as given" invites blind signing → the Receive-path independent-control property is conditional on buyer re-deriving the nonce. | Low | SPEC-2 + GOV-2 | OPEN — mark the instruction UNSAFE; require Auxilo-aware clients to fetch contributor from an authenticated source and re-derive; disclose conditionality to counsel. |
| AUD-A1 | USDC is a Circle-upgradeable proxy — every "safe" verdict is contingent on the current impl (a future upgrade could add a transfer hook / fee-on-transfer / changed nonce semantics). | — (risk register) | GOV-3 + GOV-2 | OPEN — record as explicit accepted-risk / monitoring item; largest unstated environmental assumption. |
